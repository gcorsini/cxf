...
public class TomcatHttpDestinationTest {
    ...
    public void testContinuationsIgnored() throws Exception {
        ...
        final TomcatHTTPServerEngine httpEngine = new TomcatHTTPServerEngine();
        httpEngine.setContinuationsEnabled(false);
        TomcatHTTPServerEngineFactory factory = new TomcatHTTPServerEngineFactory() {
            @Override
            public TomcatHTTPServerEngine retrieveTomcatHTTPServerEngine(int port) {
                return httpEngine;
            }
        };
        Bus b2 = new ExtensionManagerBus();
        transportFactory = new HTTPTransportFactory();
        b2.setExtension(factory, TomcatHTTPServerEngineFactory.class);

        TestTomcatDestination testDestination =
                new TestTomcatDestination(b2,
    ...
    public void testServerPolicyInServiceModel()
        ...
        engine = EasyMock.createMock(TomcatHTTPServerEngine.class);
        EasyMock.replay();
        endpointInfo.setAddress(NOWHERE + "bar/foo");

        TomcatHTTPDestination dest =
                new EasyMockTomcatHTTPDestination(
...
//            request.setHandled(true);
            ...
            if ("GET".equals(method) && "?wsdl".equals(query)) {
                ...
                for (String header : headers.keySet()) {
                    headersNames.add(header);
                }
                EasyMock.expect(request.getHeaderNames()).andReturn(Collections.enumeration(headersNames));

                request.getHeaders("content-type");
                EasyMock.expectLastCall().andReturn(Collections.enumeration(headers.get("content-type")));
...
//                request.setHandled(true);
    ...
    private void verifyBackChannelSend(Conduit backChannel,
        ...
        assertTrue("unexpected back channel type",
                backChannel instanceof TomcatHTTPDestination.BackChannelConduit);
