...
public class ApplicationContextTest {
    ...
    public void testInvalid() throws Exception {
        String s4 = getClass()
            .getResource("/org/apache/cxf/transport/http_tomcat/spring/invalid-beans.xml").toString();
    ...
    public void testContext() throws Exception {
        String s4 = getClass()
            .getResource("/org/apache/cxf/transport/http_tomcat/spring/beans.xml").toString();
    ...
    public void testContextWithProperties() throws Exception {
        String s4 = getClass()
            .getResource("/org/apache/cxf/transport/http_tomcat/spring/beans-props.xml").toString();
    ...
    private void checkContext(TestApplicationContext ctx) throws Exception {
        ...
        assertTrue(d instanceof TomcatHTTPDestination);
        TomcatHTTPDestination jd = (TomcatHTTPDestination) d;
        assertEquals("foobar", jd.getServer().getContentEncoding());

        TomcatHTTPServerEngine engine = (TomcatHTTPServerEngine)jd.getEngine();
        ...
        TomcatHTTPDestination jd2 =
            (TomcatHTTPDestination)factory.getDestination(
                getEndpointInfo("foo", "bar", "http://localhost:9001"), bus);

        engine = (TomcatHTTPServerEngine)jd2.getEngine();
...
//        assertTrue("The engine should support session manager", engine.isSessionSupport());
        ...
        TomcatHTTPDestination jd3 =
            (TomcatHTTPDestination)factory.getDestination(
                getEndpointInfo("sna", "foo", "https://localhost:9002"), bus);

        engine = (TomcatHTTPServerEngine)jd3.getEngine();
        assertEquals(111, engine.getThreadingParameters().getMinThreads());
        assertEquals(120, engine.getThreadingParameters().getMaxThreads());
        assertEquals("TestPrefix", engine.getThreadingParameters().getThreadNamePrefix());
        ...
        TomcatHTTPDestination jd4 =
            (TomcatHTTPDestination)factory.getDestination(
                getEndpointInfo("sna", "foo2", "https://localhost:9003"), bus);

        engine = (TomcatHTTPServerEngine)jd4.getEngine();
        ...
        TomcatHTTPDestination jd5 =
            (TomcatHTTPDestination)factory.getDestination(
                getEndpointInfo("sna", "foo", "http://localhost:9100"), bus);

        engine = (TomcatHTTPServerEngine)jd5.getEngine();
